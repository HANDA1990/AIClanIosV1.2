...
#define spaceMargin 10   // 为顶部留出的空白

#define kChartLineColor         [UIColor colorWithRed:200.0/255.0 green:200.0/255.0 blue:200.0/255.0 alpha:1]
#define kChartTextColor         [UIColor colorWithRed:199.0/255.0 green:47.0/255.0 blue:45.0/255.0 alpha:1]
#define PointLineColor          [UIColor colorWithRed:199.0/255.0 green:47.0/255.0 blue:45.0/255.0 alpha:1]
//#define defaultSpace 5
#define leftMargin 45
#define kScreenWidth [UIScreen mainScreen].bounds.size.width
#define lineWidthValue 0.5
#define numberOfYAxisElements 6 // y轴分为几段
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i < self.xTitleArray.count; i++) {
        ...
        if (i == 0) {
            ...
            [title drawInRect:titleRect withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:[UIColor blackColor]}];
            
            //画垂直X轴的竖线
            [self drawLine:context
                startPoint:CGPointMake(titleRect.origin.x+labelSize.width/2, self.frame.size.height - labelSize.height-5)
                  endPoint:CGPointMake(titleRect.origin.x+labelSize.width/2, 10)
    ...
    if (self.yValueArray && self.yValueArray.count > 0) {
        ...
        for (NSInteger i = 0; i < self.yValueArray.count; i++) {
            ...
            if (i == self.yValueArray.count-1) {
                ...
                CGFloat chartHeight = self.frame.size.height - textSize.height - 5 - spaceMargin;
                CGPoint endPoint = CGPointMake((i+1)*self.pointGap - 1, chartHeight -  (endValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
                
                //画最后一个点
                UIColor*aColor = PointLineColor; //点的颜色
                ...
                if (i != 0) {
                    if ((maxX + 3) > strRect.origin.x) {
                        ...
                        [str drawInRect:strRect withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:PointLineColor}];
                ...
                CGPoint startPoint = CGPointMake((i+1)*self.pointGap, chartHeight -  (startValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
                CGPoint endPoint = CGPointMake((i+2)*self.pointGap, chartHeight -  (endValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
                
                CGFloat normal[1]={1};
                CGContextSetLineDash(context,0,normal,0); //画实线
                
                CGPoint sP = CGPointMake((i+1)*self.pointGap + 1, chartHeight -  (startValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
                CGPoint eP = CGPointMake((i+2)*self.pointGap - 1, chartHeight -  (endValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
                
                [self drawLine:context startPoint:sP endPoint:eP lineColor:PointLineColor lineWidth:1.5];
    ...
    if(self.isLongPress)
    {
        ...
        if(nowPoint >= 0 && nowPoint < [self.yValueArray count]) {
            ...
            CGPoint selectPoint = CGPointMake((nowPoint+1)*self.pointGap, chartHeight -  (num.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+spaceMargin);
            ...
            if ([self isPureFloat:[num floatValue]]) {
                [[NSString stringWithFormat:@"%.1f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor blackColor]}];
            }
            else {
                [[NSString stringWithFormat:@"%.1f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15)withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor blackColor]}];
