    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            case LLImageTypeVideo:
                [self openVideo];
                break;
            case LLImageTypeTapeAndVideo:
            {
                LLActionSheetView *alert = [[LLActionSheetView alloc]initWithTitleArray:@[@"录像", @"视频"] andShowCancel: YES];
                alert.ClickIndex = ^(NSInteger index) {
                    NSLog(@"%zd",index);
                    if (index == 2) {
                        [weakSelf openVideo];
...
//        picker.cameraDevice = UIImagePickerControllerCameraDeviceRear;
        ...
        picker.videoMaximumDuration = 5.0f; //录像最长时间
        picker.allowsEditing = YES;
    } else {
